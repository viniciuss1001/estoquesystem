...
import { useLateDeliveries } from "@/lib/queries"
...
const LateDeliveries = () => {
	...
	const { data, isLoading } = useLateDeliveries()
		...
		<Card className="col-span-1 sm:col-span-2 lg:col-span-3 min-w-[350px] w-fit h-fit min-h-[200px] transition-all duration-200 hover:shadow-md hover:-translate-y-1 bg-background">
			...
			<CardContent>
				{isLoading ? (
							...
							<li key={delivery.id} className="flex items-start justify-between">
								<div className="space-y-1">
									<div className="flex items-center gap-2">
										...
										<Badge variant="destructive">{delivery.quantity} un.</Badge>
									</div>
									<div className="text-sm text-muted-foreground">
										Fornecedor: <strong>{delivery.supplier.name}</strong> — Venc.{" "}
					...
					<p className="text-sm text-muted-foreground">Nenhuma entrega em atraso.</p>
				)}
			</CardContent>
		</Card>
	)
